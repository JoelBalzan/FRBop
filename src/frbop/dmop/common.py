...
def add_asym(x0s,siglos,sighis,order=2,nmax=100,tol=1e-6,ohwell=False):
	...
	https://ui.adsabs.harvard.edu/abs/2019A%26A...627A..84L/abstract
	adapting 
	https://ui.adsabs.harvard.edu/abs/2003physics...6138B/abstract
	Purpose
	-------
	...
	assert order in [0, 1, 2], (
		"\nParameter `order` must be 1 or 2 "
		"(or 0, but... well, stick to 1 or 2)."
	)
	x0s = np.asarray(x0s)
	...
	if order == 0:
		...
		sighi = sqrt(np.sum(sighis**2))
	else:
		# ------------------ First, calculate total moments ------------------
		mu, V, gamma = 0.0, 0.0, 0.0
		if order == 1:
			sig = (sighis + siglos) / 2.0
			alpha = (sighis - siglos) / 2.0
			mu = np.sum(x0s + (sighis - siglos) / sqrt2pi)
			V = np.sum(sig**2 + alpha**2 * (1 - 2 / pi))
			t1 = sighis**3 - siglos**3
			t2 = (sighis - siglos) * (sighis**2 + siglos**2)
			t3 = (sighis - siglos) ** 3
			gamma = np.sum(2.0 * t1 - 1.5 * t2 + t3 / pi) / sqrt2pi
		else:  # order == 2
			sig = (sighis + siglos) / 2.0
			alpha = (sighis - siglos) / 2.0
			mu = np.sum(x0s + alpha)
			V = np.sum(sig**2 + 2 * alpha**2)
			gamma = np.sum(6 * sig**2 * alpha + 8 * alpha**3)
		# ----------------- Check if iteration is necessary ------------------
		check = abs(gamma / mu**3) if mu != 0.0 else gamma
		if check < 1e-12:
			x0 = mu
			siglo = sqrt(V)
			sighi = siglo
			...
			if order == 1:
				...
				while True:
					...
					D = (
						2 / (3.0 * S)
						* (sqrt2pi * gamma - D**3 * (1 / pi - 1))
					)
					if Dold != 0.0:
						if abs(D / Dold - 1.0) < tol:
							break
					assert n < nmax, "Too many iterations"
				S = 2 * V + D**2 / pi
				x0 = mu - D / sqrt2pi
				alpha = D / 2.0
				sig = sqrt(V - alpha**2 * (1 - 2 / pi))
				siglo = sig - alpha
				sighi = sig + alpha
			elif order == 2:
				alpha = 0.0
				while True:
					n += 1
					aold = alpha
					alpha = gamma / (6 * V - 4 * alpha**2)
					if aold != 0.0:
						if abs(alpha / aold - 1.0) < tol:
							break
					assert n < nmax, "Too many iterations"
				sig = sqrt(V - 2 * alpha**2)
				x0 = mu - alpha
				siglo = sig - alpha
				sighi = sig + alpha

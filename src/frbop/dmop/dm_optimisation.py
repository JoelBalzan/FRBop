...
class DMOptimiser:
	...
	def plot_comparison(self, results: Dict, dm_range: Tuple[float, float],
		...
		axes[0, 0].set_title(f"Original Data (SHRINE structure-maximised)\nInput DM = {self._format_dm(self.input_dm, 3)} pc cm⁻³")
		...
		if len(method_names) > 0:
			...
			all_scan_ax.set_xlabel('DM (pc cm⁻³)')
	...
	def plot_component_dm_diagnostics(self,
		...
		# One-column size for typical two-column journal layouts (~3.4 in wide)
		...
		ax.set_ylabel('Best DM (pc cm⁻³)', fontsize=axis_label_fs)
		ax.grid(True, alpha=0.3)
		...
		ax.set_xticklabels(component_names, fontsize=tick_fs)
		x_pad = 0.2

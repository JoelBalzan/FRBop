...
def lowpass_smooth(CI_data: np.ndarray, kc: int, order: int = 3):
    """
    Apply the SHRINE low-pass filter to DCT-domain data.

    :param CI_data: FRB data in DCT space, shape (dm, k)
    :type CI_data: :class:`np.ndarray`
    :param kc: Cutoff index for the low-pass response
    :type kc: int
    :param order: Filter order O in fL = 1/(1+(k/kc)^(2O))
    :type order: int
    :return: Smoothed data in time domain plus intermediate filter products
    :rtype: tuple
    """
    ...
    fL = 1 / (1 + (k / kc) ** (2 * O))
    fL_diag = np.diag(fL)
    LPF_data = fL_diag @ np.transpose(CI_data)
    I_smooth = idct(np.transpose(LPF_data), norm='ortho')
    return I_smooth, LPF_data, fL_diag, fL

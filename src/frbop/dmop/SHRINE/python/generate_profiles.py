...
def _main():
    ...
    for DM in DMs:
        I_red, Q_red, U_red = do_DM(fft(X), fft(Y), DM, freqs, dt, bw)
        I_profs.append(I_red)
        Q_profs.append(Q_red)
        U_profs.append(U_red)

    I_profs = np.array(I_profs)
    #I_profs = np.transpose(np.transpose(I_profs)[3000:8000])

    I_profs = np.array(I_profs)
    Q_profs = np.array(Q_profs)
    U_profs = np.array(U_profs)

    np.save(f"{args.label}_DMs.npy", DMs)
    np.save(f"{args.label}_I_{dt}us.npy", I_profs)
    np.save(f"{args.label}_Q_{dt}us.npy", Q_profs)
    np.save(f"{args.label}_U_{dt}us.npy", U_profs)

    # Save PA computed from Q and U (radians)
    pa_profs = 0.5 * np.arctan2(U_profs, Q_profs)
    np.save(f"{args.label}_PA_{dt}us.npy", pa_profs)
...
def do_DM(X_f, Y_f, DM, freqs, dt, bw):
    ...
    # Stokes parameters from dynamic spectra (per time, per channel)
    I_ds = np.abs(X_ds) ** 2 + np.abs(Y_ds) ** 2
    Q_ds = np.abs(X_ds) ** 2 - np.abs(Y_ds) ** 2
    U_ds = 2.0 * np.real(X_ds * np.conjugate(Y_ds))

    # normalise channels independently for each Stokes-like array
    for i in range(I_ds.shape[1]):
        I_ds[:, i] -= np.mean(I_ds[:, i])
        I_ds[:, i] /= np.std(I_ds[:, i])

        Q_ds[:, i] -= np.mean(Q_ds[:, i])
        Q_ds[:, i] /= (np.std(Q_ds[:, i]) if np.std(Q_ds[:, i]) != 0 else 1.0)

        U_ds[:, i] -= np.mean(U_ds[:, i])
        U_ds[:, i] /= (np.std(U_ds[:, i]) if np.std(U_ds[:, i]) != 0 else 1.0)

    # sum over channels (drop last column as original code did)
    I_dd = I_ds[:, :-1].sum(axis=1)
    Q_dd = Q_ds[:, :-1].sum(axis=1)
    U_dd = U_ds[:, :-1].sum(axis=1)

    # time-scrunch to requested dt
    I_red = tscrunch(I_dd, dt)
    Q_red = tscrunch(Q_dd, dt)
    U_red = tscrunch(U_dd, dt)

    return I_red, Q_red, U_red
